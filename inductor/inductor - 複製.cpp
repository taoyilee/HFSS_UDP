...
//	struct UDPPosition boxP = {OD+P,0,zo};
//	double boxS[3] = {lead,-W,thick};
//	functionLib->createBox(&boxP, boxS, callbackData);

//	for (int i =0; i< NT-1;i++){
//		path = octt(functionLib, callbackData, paramValues, &startPt, &endPt,OD-i*2*P,W,S,i*P+xo,yo,zo, DIV, grid);
//		if (path == -1)		{
...
//		
//	}
//	path = octtl(functionLib, callbackData, paramValues, &startPt, &endPt,OD-(NT-1)*2*P,W,S, (NT-1)*P+xo,yo,zo, DIV, grid);
//	// Is path valid?
//	if (path == -1)	{
//		functionLib->addMessage(kUDPErrorMessage, "Could not create path", callbackData);
//		return 0;
...
double OD, double W, double S, double xoff, double yoff, double zoff,double DIV, double maskGrid){
	double P=S+W;
