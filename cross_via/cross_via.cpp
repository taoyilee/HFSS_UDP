...
long CreatePrimitive(struct UDPFunctionLib* functionLib, void* callbackData, double* paramValues)
{
	...
	long path;
		path = cross1(functionLib, callbackData, paramValues, &startPt, &endPt);
		// Is thicken polygon valid?
        path = functionLib->sheetThicken(path,thick,0,callbackData);
		if ( path == -1)		{
			functionLib->addMessage(kUDPErrorMessage, "Sheet Thickening Failed", callbackData);
			return 0;
		}	
    path = functionLib->createBox(&startPt2, size, callbackData);
    path = functionLib->createBox(&startPt3, size, callbackData);
	return path;
}
// User defined extra functions
long cross1(struct UDPFunctionLib* functionLib, void* callbackData, double* paramValues, struct UDPPosition* startPt, struct UDPPosition* endPt){
	double CONST_Z = 0.0;
	int noOfPoints = 8;
	...
	double W = paramValues[3];
	double S = paramValues[4];
	double OPENING = paramValues[5];
	double maskGrid = paramValues[7];
