...
double roundtogrid(double, double);
static struct UDPPrimitiveTypeInfo primitiveInfo = {
	"Octagonal Spiral",
	"Create an octagonal spiral in XY plane",
...
long CreatePrimitive(struct UDPFunctionLib* functionLib, void* callbackData, double* paramValues)
{
	...
	if(NT > 1){ // make a space between lead and spiral
		...
		boxP.y = S -viaen;
		boxP.z = zo;
		boxS[0] = lead+W; boxS[1] = W; boxS[2] = thick;
		functionLib->createBox(&boxP, boxS, callbackData);
...
double roundtogrid(double x, double maskGrid){
       return maskGrid*round(x/maskGrid);
       }
...
long octt(struct UDPFunctionLib* functionLib, void* callbackData, double* paramValues, struct UDPPosition* startPt, struct UDPPosition* endPt, int i){
	...
	double C= roundtogrid(W*tan(PI/8), paramValues[9]);
	double A= roundtogrid(OD/DIV, paramValues[9]);
	double B=OD-2*A;
	double BA = roundtogrid(B/2, paramValues[9]);
	double BB=B-BA;
	int noOfPoints = 20;
	int noOfSegments = noOfPoints;
	struct UDPPosition*  arrayOfPositions = new UDPPosition[noOfPoints];
	/*OD:0            OD-W:0 */
	arrayOfPositions[0].x = OD-S;	
	arrayOfPositions[0].y = -viaen;
	arrayOfPositions[1].x = OD-W-S;
	arrayOfPositions[1].y = -viaen;
	/*OD-W:BA         OD-A:BA+A-W*/
	arrayOfPositions[2].x = OD-W-S;
	arrayOfPositions[2].y = BA;
	arrayOfPositions[3].x = OD-A-S;
	...
	arrayOfPositions[8].x = OD+P-A-S;
	arrayOfPositions[8].y = -BB-A+W;
	arrayOfPositions[9].x = OD;
	arrayOfPositions[9].y = -BB;
	/*OD+S:0        OD+P:0*/
	arrayOfPositions[10].x = OD ;
	arrayOfPositions[10].y = -viaen;
	arrayOfPositions[11].x = OD+W;
	arrayOfPositions[11].y = -viaen;
	/*OD+P:-BB-C     OD+P-A+C:-BB-A*/
	arrayOfPositions[12].x = OD+W;
	arrayOfPositions[12].y = -BB-C  ;
	arrayOfPositions[13].x = OD+W-A+C;
	...
	/*OD-A+C:BA+A     OD:BA+C*/
	arrayOfPositions[18].x = OD-A+C-S;
	arrayOfPositions[18].y = BA+A;
	arrayOfPositions[19].x = OD-S;
