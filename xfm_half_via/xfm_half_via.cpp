...
static struct UDPPrimitiveTypeInfo primitiveInfo = {
	"Transformer Half",
	"Sub-Component of a Transformer (with VIAs)",
...
struct UDPPrimitiveParameterDefinition primParams[] = {
	...
	{"VIAW", "Tunging Factor", kUDPLengthUnit, 2}, // 10
	{"VIAH", "Tunging Factor", kUDPLengthUnit, 2}, // 11    	
};
...
long CreatePrimitive(struct UDPFunctionLib* functionLib, void* callbackData, double* paramValues)
{
	...
	long polygon;	
    long polygon2;
		path = half(functionLib, callbackData, paramValues, &startPt, &endPt);
...
long half(struct UDPFunctionLib* functionLib, void* callbackData, double* paramValues, struct UDPPosition* startPt, struct UDPPosition* endPt){
	double xoff = paramValues[0];
	double yoff = paramValues[1];
	double zoff = paramValues[2];
	double OD = paramValues[3];
	double W = paramValues[4];
	double L_OP = paramValues[5];
	double R_OP = paramValues[6];
	...
	double DIV = paramValues[9];
	double maskGrid = paramValues[8];
	double VIAW = paramValues[10];
	double VIAH = paramValues[11];
	char str[30];
	double CONST_Z = 0.0;
	int noOfPoints = 12;
    ...
    double size[3] = {VIAW,VIAW,VIAH};
	struct UDPPosition viaPt = {xoff, yoff,zoff+thick};
	int count =0;
    count = (int) floor((BA + VIAW - R_OP/2)/(2*VIAW));
    double encl = (W - VIAW)/2;
    for(int i=0; i< count; i++){
        viaPt.x = xoff + encl ;
        viaPt.y = yoff + encl + R_OP/2 + i*2*VIAW;    
        functionLib->createBox(&viaPt, size, callbackData);
    }
    count = (int) floor((BA + VIAW - L_OP/2)/(2*VIAW));
    for(int i=0; i< count; i++){
        viaPt.x = xoff + OD - W + encl;
        viaPt.y = yoff + encl +L_OP/2 + i*2*VIAW;    
        functionLib->createBox(&viaPt, size, callbackData);
    }
    count = (int) floor((B+2*C+VIAW)/(2*VIAW));
    for(int i=0; i< count; i++){
        viaPt.x = xoff + A - C + i*2*VIAW;
        viaPt.y = yoff + BA+A - W + encl;    
        functionLib->createBox(&viaPt, size, callbackData);
    }
