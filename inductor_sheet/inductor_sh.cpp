...
struct UDPPrimitiveParameterDefinition primParams[] = {
	...
	{"DIV", "Shape Turning Factor", kUDPNoUnit, 3.14}, // 9
    {"VIA_ENC", "via enclosure", kUDPLengthUnit, 0} // 10
};
...
long CreatePrimitive(struct UDPFunctionLib* functionLib, void* callbackData, double* paramValues)
{
	...
	double grid = paramValues[8];
	double DIV = paramValues[9];
	double viaen = paramValues[10];
	long path;
	long polygon;
	double P = S + W;
	
//	struct UDPPosition boxP = {OD+P,-viaen,zo};
//	double boxS[2] = {lead,-W};
//	functionLib->createRectangle(kUDPXYPlane,&boxP, boxS,1, callbackData);
